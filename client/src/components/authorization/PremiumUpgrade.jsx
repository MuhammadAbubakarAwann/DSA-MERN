...
const PremiumUpgrade = () => {
  ...
  const handlePaymentClick = async () => {
    ...
    try {
      ...
      if (result.error) {
        dispatch(showToast({ type: 'error', message: result.error.message }));


      }
    } catch (error) {
      console.error('Error initiating payment:', error);
      dispatch(showToast({ type: 'error', message: error.message || "Payment initiation failed." }));
  ...
  return (
    <div>
      <abbr title="upgrade to premium to get full access to advance data structures">
